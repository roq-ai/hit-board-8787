...
import { deleteParticipantById, createParticipant } from 'apiSdk/participants';
...
function OrganizationViewPage() {
  ...
  const participantHandleCreate = async () => {
    setCreateError(null);
    try {
      await createParticipant({ organization_id: id, user_id: participantUserId });
  ...
  const participantHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
                ...
                <Flex gap={5} alignItems="flex-end">
                  <Box flex={1}>
                    <UserSelect
                      ...
                      handleChange={setParticipantUserId}
                    />
                  </Box>
                  <Button
                    colorScheme="blue"
                    mt="4"
                    mr="4"
                    onClick={participantHandleCreate}
                    isDisabled={!participantUserId}
                  >
                    Create
                  </Button>
                </Flex>
                          ...
                          <Td>
                            {hasAccess('user', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                              <IconButton
                                onClick={(e) => {
                                  e.stopPropagation();
                                  participantHandleDelete(record.id);
                                }}
                                colorScheme="red"
                                variant="outline"
                                aria-label="edit"
